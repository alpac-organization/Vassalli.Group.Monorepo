...
export class AreasServices implements IAreasServices {
	...
	public async getAreas(payload: GetAreasRequest): Promise<GetAreasResponse[]> {
		try {
			...
			const areas = await this.apiHandler.get<GetAreasResponse[]>(`/companies/${company_id}/modules/${module_code}/areas`);
			return areas;
		} 
		catch (error) {
			throw error;
	...
	public async createArea(payload: CreateAreaRequest): Promise<void> {
		try {
			const { company_id, module_code, work_area_name, description } = payload;
			...
			const area = await this.apiHandler.post<void>(`/companies/${company_id}/modules/${module_code}/areas`, body);
			return area;
		} 
		catch (error) {
			throw error;
	...
	public async deleteArea(payload: DeleteAreaRequest): Promise<void> {
		try {
			const { company_id, module_code, area_id } = payload;

			await this.apiHandler.delete<void>(`/companies/${company_id}/modules/${module_code}/areas/${area_id}`);
		} 
		catch (error) {
			throw error;
		}
