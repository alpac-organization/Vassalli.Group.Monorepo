...
export class CostCentersServices implements ICostCentersServices {
	...
	public async getCostCenters(payload: GetCostCentersRequest): Promise<GetCostCentersResponse[] | undefined> {
		try {
			const { company_id, area_id,module_code } = payload;

			const costCenters = await this.apiHandler.get<GetCostCentersResponse[] | undefined>(`/companies/${company_id}/modules/${module_code}/areas/${area_id}/cost-centers`);
