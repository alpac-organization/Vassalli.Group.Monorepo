...
import { UnloadingDetailsModal } from "./components/unloading-details-modal/unloading-details-modal";
...
export const MerchandiseUnloadingControl = function () {
   ...
   const [pendingAssigment, setPendingAssigment] = useState<PendingAssignment | null>(null);
   ...
   const handleViewDetail = useCallback((data: PendingAssignment) => {
      setPendingAssigment(data);
   }, []);

   const lastItemId = rows.at(-1)?.assignment_id;

   const columns = useMemo(
      () => getMerchandiseUnloadingColumns({ onViewDetail: handleViewDetail, lastItemId }),
      [handleViewDetail, lastItemId],
   );
   ...
   return (
      <>
         ...
         {(isLoading || isFetching) && <Loader title="Cargando descargues..." />}
         ...
         <UnloadingDetailsModal
            isOpen={Boolean(pendingAssigment)}
            pendingAssignment={pendingAssigment}
            onClose={() => setPendingAssigment(null)}
         />
