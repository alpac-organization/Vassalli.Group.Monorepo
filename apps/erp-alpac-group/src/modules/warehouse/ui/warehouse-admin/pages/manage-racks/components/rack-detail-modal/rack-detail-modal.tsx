...
import { RackUsageProfileEnum } from "@app/modules/warehouse/domain/enums/rack-usage-profile.enum";
import { RackStatusBadge } from "@app/modules/warehouse/ui/warehouse-admin/utils/layout-badges";

interface RackDetailModalProps {
	isOpen: boolean;
	rack?: RackDetailResponse | null;
	isLoading: boolean;
	onClose: () => void;
}
...
const getUsageProfileLabel = (value: string | null) => {
	if (!value) return "-";
...
export const RackDetailModal = ({ isOpen, rack, isLoading, onClose }: RackDetailModalProps) => {
	const positions = [...(rack?.positions ?? [])].sort(
		(left, right) => left.position_number - right.position_number,
	);

	const heightLabel = rack?.height_metres != null ? ` x ${rack.height_metres}` : "";
		...
		>
			...
			{!isLoading && rack && (
				<div className="flex flex-col gap-5">
					<div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
						<div className="rounded-lg border border-[#2a2d3d] bg-[#1b1e27] p-3">
							...
							<p className="mt-1 text-sm font-medium text-white">
								{rack.width_metres} x {rack.length_metres}
								{heightLabel} m
							</p>
						</div>
					...
					{rack.unavailable_reason && (
						<div className="rounded-lg border border-[#3a2c0a] bg-[#33270a] p-3 text-sm text-[#fbbf24]">
							<strong>Motivo:</strong> {rack.unavailable_reason}
						</div>
					)}

					<div>
						<div className="mb-2 flex items-center justify-between">
							<p className="text-sm font-medium text-slate-300">Distribución de posiciones</p>
						...
						<div className="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
							{positions.map((position) => (
								<div
									...
									title={position.block_reason || position.position_code}
