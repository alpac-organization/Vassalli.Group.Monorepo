...
export const LotDetailModal = ({ isOpen, lot, isLoading, onClose }: LotDetailModalProps) => {
	const positionsByRowColumn = new Map<string, LotDetailResponse["positions"][number]>();

	if (lot) {
		for (const position of lot.positions) {
			positionsByRowColumn.set(`${position.row_number}-${position.column_number}`, position);
		}
	}
	...
	const renderPosition = (rowNumber: number, columnNumber: number) => {
		...
		if (!position) {
			return (
				...
				>
					<span>{rowNumber}-{columnNumber}</span>
				</div>
			);
		}
				...
				<div className="flex flex-col gap-5">
					...
					<div>
						...
						<div
							...
							style={{ gridTemplateColumns: `repeat(${lot.nominal_columns}, minmax(0, 1fr))` }}
						>
							{Array.from({ length: lot.nominal_rows }, (_, rowIndex) => rowIndex + 1).map(
								(rowNumber) =>
									Array.from(
