...
import { RackUsageProfileEnum, RackUsageProfileOptions } from "@app/modules/warehouse/domain/enums/rack-usage-profile.enum";
import type { CreateRacksRequest, RackLevelSpecRequest } from "@app/modules/warehouse/domain/ApiContract/Requests/warehouse-requests/create-racks-request";
import {
	formatAmount,
...
export const RackModal = ({ isOpen, sectionId, onClose, onSubmit }: RackModalProps) => {
	...
	const { CreateRacks } = useWarehouseLayout();
						...
						{...register("starting_deposit_number", {
							validate: {
								validateInteger: (value) =>
									value === undefined || value === null || value === "" || validateIntegerNumber(value),
								validatePositive: (value) =>
									value === undefined || value === null || value === "" || validatePositiveNumber(value),
							},
								...
								<div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
									...
									{showUnavailableReason && (
										<InputText
											...
											error={errors.levels?.[index]?.unavailable_reason?.message}
										/>
									)}
								</div>
							</div>
