...
export const Depreciation = () => {
   ...
   const {
      onChange: onDepreciationAmountChange,
      ...depreciationAmountField
   } = register("depreciation_payload.depreciation_amount", {
      required: "El monto de la depreciación es requerido",
   ...
   return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
         <Controller
            name="depreciation_payload.currency"
            control={control}
            rules={{
            ...
            render={({ field }) => (
               <Dropdown
                  ...
                  error={
                     errors.depreciation_payload?.currency?.message as string
                  }
                  value={field.value}
                  appearance="dark"
         ...
         <InputText
            label="Monto de la depreciación"
            type="text"
            inputMode="decimal"
            placeholder="0.00"
            ...
            {...depreciationAmountField}
            error={
               errors.depreciation_payload?.depreciation_amount
                  ?.message as string
            }
            onChange={(evt) => {
               evt.target.value = formatAmount(evt.target.value, 18, 2);
               onDepreciationAmountChange(evt);
            }}
         />
      </div>
   );
