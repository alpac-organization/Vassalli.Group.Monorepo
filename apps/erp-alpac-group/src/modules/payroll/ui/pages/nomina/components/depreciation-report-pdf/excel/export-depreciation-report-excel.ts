...
export type ExportDepreciationReportExcelParams = {
...
const COLUMNS = [
  { key: "code", label: "Idemplea", width: 14 },
  { key: "name", label: "Nombre", width: 36 },
  { key: "cordobas", label: "Córdobas", width: 18 },
  { key: "dolares", label: "Dólares", width: 18 },
  { key: "observation", label: "Observación", width: 32 },
] as const;
...
export async function exportDepreciationReportExcel({
  data,
  branchName,
  startDate,
  endDate,
  logoUrl,
}: ExportDepreciationReportExcelParams): Promise<void> {
  const { Workbook } = await import("exceljs");

  const reportTitle = "Reporte de Depreciaciones";

  const wb = new Workbook();
  wb.creator = "ALPAC ERP";

  const ws = wb.addWorksheet("Depreciaciones", {
    pageSetup: {
  ...
  {
    const periodLabel =
      buildDepreciationReportPeriodLabel(startDate, endDate) ??
  ...
  if (logoUrl) {
    try {
      ...
      console.warn("No se pudo agregar el logo en Reporte de Depreciaciones");
    }
  }
  ...
  link.download = `reporte-depreciaciones-${branchSlug}-${now}.xlsx`;
