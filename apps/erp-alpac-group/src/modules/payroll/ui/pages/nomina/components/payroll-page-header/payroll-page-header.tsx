...
export default function PayrollPageHeader({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {branchName ? (
        <div className="mt-3 flex flex-col items-center gap-3 sm:mt-1 sm:gap-2">
          <Badges
            label={`Nomina de ${branchName}`}
            color="bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300"
            className="max-w-72 text-[13px]! sm:text-[13px]! font-semibold! leading-snug! wrap-break-word text-center mb-2"
          />
          {onRequestChangePayrollSelection ? (
            <Button
              type="button"
              ...
              className="w-full! min-h-[48px]! px-4! text-center! text-[15px]! leading-snug! rounded-md! text-white! bg-slate-500! dark:bg-slate-700! lg:w-auto!"
            />
          ) : null}
        </div>
      ) : null}
