...
export class PermissionServices implements IPermissionRequestServices {
   ...
   public async getPermissionHistory(payload: PermissionHistoryRequest): Promise<PermissionHistoryResponse[]> {
      try {
   ...
   public async cancelPermissionRequest(payload: CancelPermissionRequest): Promise<void> {
      try {
   ...
   public async generatePermissionDocument(payload: GeneratePermissionDocumentRequest): Promise<void> {
      try {
         const { company_id, module_code, permit_application_id } = payload;
         const response = await this.apiHandler.get<void>(
