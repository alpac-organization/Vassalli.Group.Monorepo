...
export const useQuoteAnalysis = (props: UseQuoteAnalysisProps) => {
	...
	const GetQuoteAnalysis = useQuery<GetRequisitionAccountingReviewsResponse, ApiErrorResponse>({
		queryKey: ["quotes-analysis", payloadGetQuoteAnalysis],
		queryFn: () => quoteAnalysisService.GetQuoteAnalysis(payloadGetQuoteAnalysis as GetQuotesAnalysisRequest),
	...
	const GetQuoteAnalysisDetails = useQuery<RequisitionAccountingReviewDetailsDto, ApiErrorResponse>({
		queryKey: ["quotes-analysis-details", payloadGetQuoteAnalysisDetails],
		queryFn: () => quoteAnalysisService.GetQuoteAnalysisDetails(payloadGetQuoteAnalysisDetails as GetQuoteAnalysisDetailsRequest),
	...
	const AcceptQuotationToPurchase = useMutation<void, ApiErrorResponse, AcceptOfferPurchaseRequest>({
		mutationFn: (payload: AcceptOfferPurchaseRequest) => quoteAnalysisService.accceptQuotationToPurchase(payload),
		onSuccess: () => {
	...
	const SendReviewToManagement = useMutation<void, ApiErrorResponse, SendReviewToManagementRequest>({
		mutationFn: (payload: SendReviewToManagementRequest) => quoteAnalysisService.sendReviewToManagement(payload),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["quotes-analysis"] });
			queryClient.invalidateQueries({ queryKey: ["quotes-analysis-details"] });
		},
