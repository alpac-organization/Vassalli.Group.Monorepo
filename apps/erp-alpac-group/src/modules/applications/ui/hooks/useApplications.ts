...
export const useApplications = (
  filters?: ApplicationRequest,
  config?: { enabled?: boolean; enabledDetail?: boolean },
) => {
  const queryClient = useQueryClient();

  type ApplicationResponse = Awaited<
    ReturnType<ApplicationServices["GetApplications"]>
  >;

  type ApplicationDetailResponse = Awaited<
    ReturnType<ApplicationServices["GetApplicationDetail"]>
  >;

  type ProcessApplicationResponse = Awaited<
    ReturnType<ApplicationServices["ProcessApplication"]>
  >;
  ...
  >({
    mutationFn: (payload: ApplicationProcessRequest) =>
      applicationServices.ProcessApplication(payload),
  ...
  >({
    ...
    enabled:
      config?.enabledDetail !== undefined ? config.enabledDetail : !!filters,
    refetchOnMount: true,
    refetchOnWindowFocus: false,
    staleTime: 0,
    retry: 1,
